...
import com.yxb.mall.architect.constant.BusinessConstants;
import com.yxb.mall.architect.constant.BussinessCode;
import com.yxb.mall.architect.utils.BussinessMsgUtil;
import com.yxb.mall.architect.utils.KeyConfig;
import com.yxb.mall.dao.ResourceMapper;
import com.yxb.mall.dao.RoleResourceMapper;
import com.yxb.mall.domain.bo.BussinessMsg;
import com.yxb.mall.domain.bo.Tree;
import com.yxb.mall.domain.dto.ResourceChildrenMenuDto;
import com.yxb.mall.domain.dto.ResourceMenuDto;
import com.yxb.mall.domain.vo.Resource;
import com.yxb.mall.domain.vo.RoleResource;
import com.yxb.mall.domain.vo.UserRole;
...
public class ResourceService {
     ...
     * 顶部菜单展示封装
     * @return
     * param userIdCondition 用户id条件
     */
    public String selectResMenuTop(String userIdCondition){
        List<UserRole> userRoleList=roleService.getRoleIdsByUserId(userIdCondition);//获取用户角色
        List<Integer> roleIdList = new ArrayList<>();
        List<ResourceMenuDto>  lastResourceMenuDto= new ArrayList<>();//最终存放菜单
        for(UserRole userRole:userRoleList){
            roleIdList.add(userRole.getRoleId());//获取所有角色id
        }
        List<RoleResource> roleResourcesList = roleResourceMapper.getRoleResources(roleIdList);//根据角色查回来角色菜单表里的菜单
        if(CollectionUtils.isEmpty(roleResourcesList)){
            return null;
        }
        List<ResourceMenuDto> parentResList =  resourceMapper.selectParentIdResList();//顶部菜单
        if(CollectionUtils.isEmpty(parentResList)){
            return null;
        }
        for(ResourceMenuDto resourceMenuDto:parentResList){
            for(RoleResource roleResource:roleResourcesList){
                if(resourceMenuDto.getPid().equals(roleResource.getResourceId())){
                    lastResourceMenuDto.add(resourceMenuDto);
                    continue;
                }
            }
        }

        if(null != lastResourceMenuDto && !lastResourceMenuDto.isEmpty()){
            HashSet h = new HashSet(lastResourceMenuDto);//去重
            lastResourceMenuDto.clear();
            lastResourceMenuDto.addAll(h);
            return Json.toJson(lastResourceMenuDto, JsonFormat.full());
        }
        return null;
