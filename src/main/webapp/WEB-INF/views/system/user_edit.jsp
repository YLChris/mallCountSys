...
<form class="layui-form layui-form-pane">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="text" class="layui-input" name="userName" lay-verify="required|userName" maxlength="20" value="${user.userName}" placeholder="请输入员工姓名">
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">薪水</label>
        <div class="layui-input-block">
            <input type="number" class="layui-input" name="salary" lay-verify="required|salary" value="${user.salary}" placeholder="请输入员工薪水">
        </div>
    </div>
    <div class="layui-form-item" pane>
        ...
        <div class="layui-input-block">
            <c:if test="${pageFlag == 'addPage' }">
                <input type="radio" name="userStatus" value="0" title="在职" checked disabled>
                <input type="radio" name="userStatus" value="1" title="离职" disabled>
            </c:if>
            <c:if test="${pageFlag == 'updatePage' and  user.userStatus == '0' }">
                <input type="radio" name="userStatus" value="0" title="在职"  disabled <c:if test="${user.userStatus ==0 }">checked</c:if>/>
                <input type="radio" name="userStatus" value="1" title="离职" disabled  <c:if test="${user.userStatus ==1 }">checked</c:if>/>
            </c:if>
            <c:if test="${pageFlag == 'updatePage' and  user.userStatus == '1' }">
                <input type="radio" name="userStatus" value="0" title="在职"   <c:if test="${user.userStatus ==0 }">checked</c:if>/>
                <input type="radio" name="userStatus" value="1" title="离职"   <c:if test="${user.userStatus ==1 }">checked</c:if>/>
            </c:if>
        </div>
    </div>
